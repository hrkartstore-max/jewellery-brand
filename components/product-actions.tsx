  ...
  return (
    <>
            ...
            <div className="flex gap-2">
              {product.sizes.map((size) => (
                <button
                  ...
                  className={`w-11 h-11 border transition ${selectedSize === size ? 'border-black bg-black text-white' : 'border-line hover:border-black'}`}
                >
                  {size}
                </button>
              ))}
      ...
      <div className="flex gap-3 mt-8">
        ...
        <button onClick={() => toggleWishlist(product.id)} className="w-14 border border-line" aria-label="Wishlist">
          <Heart size={19} className="mx-auto" fill={wishlisted ? 'currentColor' : 'none'} />
        </button>
      </div>

      <button onClick={add} className="w-full mt-3 h-14 border border-black text-[10px] tracking-[.18em]">BUY NOW</button>
