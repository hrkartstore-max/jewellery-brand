...
export default function Cart() {
  ...
  if (!cart.length) {
    return (
      <main className="max-w-[1000px] mx-auto px-5 py-24 text-center">
        <div className="text-[10px] tracking-[.22em] text-gold">YOUR AURELIA EDIT</div>
  ...
  return (
    <main className="max-w-[1200px] mx-auto px-5 md:px-8 py-16 md:py-24">
      <div className="text-[10px] tracking-[.22em] text-gold">YOUR AURELIA EDIT</div>
      <h1 className="font-display text-6xl mt-3">Your Cart</h1>
      <div className="grid lg:grid-cols-[1fr_360px] gap-12 mt-12">
        <div className="divide-y border-y border-line">
          {cart.map((item) => (
            <div key={`${item.id}-${item.selectedSize ?? ''}`} className="py-6 flex gap-5">
              <Link href={`/products/${item.slug}`} className="relative w-28 md:w-36 aspect-[4/5] bg-sand shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href={`/products/${item.slug}`} className="font-display text-2xl block mt-1">{item.name}</Link>
                ...
                <div className="flex items-center gap-3 mt-5">
                  <div className="flex border border-line h-10 items-center">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3" aria-label="Decrease"><Minus size={13}/></button>
                    <span className="min-w-8 text-center text-sm">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-3" aria-label="Increase"><Plus size={13}/></button>
                  </div>
                  <button onClick={() => removeFromCart(item.id)} className="text-black/45 hover:text-black" aria-label="Remove"><Trash2 size={16}/></button>
                </div>
              </div>
            </div>
          ))}
